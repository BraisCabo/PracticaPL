...
sentlist returns [String text]: mainhead LBRACE code RBRACE {$text="<A NAME=\"Main\">\n<code>\n"+$mainhead.text+"\n{"+"\n}\n" +"\n<br/>\n</code>\n"+
...
ctelist
	returns[String text]:
	DEFINE CONST_DEF_IDENTIFIER simpvalue {$text= "<SPAN CLASS=\"palres\">" +  $DEFINE.text + "</SPAN> " + "<A NAME=\""+ $CONST_DEF_IDENTIFIER.text + "\">" + "<SPAN CLASS=\"ident\">" + $CONST_DEF_IDENTIFIER.text + "</SPAN> " +"<SPAN CLASS=\"cte\">"+ $simpvalue.text + "</SPAN>";
		};
simpvalue
	returns[String text]:
...
vardef
	returns[String text]:
	tbas IDENTIFIER vardef2 {$text= "<SPAN CLASS=\"palres\">" + $tbas.type + "</SPAN> " + "<A NAME=\""+ $IDENTIFIER.text + "\">" + "<SPAN CLASS=\"ident\">" + $IDENTIFIER.text + "</SPAN>" +  $vardef2.text;
		};
vardef2
	returns[String text]:
	SASIGN simpvalue {$text= "<SPAN CLASS=\"palres\"> " + $SASIGN.text  + "</SPAN> " + "<SPAN CLASS=\"cte\">"+ $simpvalue.text + "</SPAN>";
		}
	| {$text="";};
tbas
	returns[String type]:
...
funcdef
	...
	funchead LBRACE code RBRACE {$body = "<A NAME=\""+ $funchead.name + "\">\n"
		+ "<code>\n" + $funchead.text + "\n{\n" + "}\n" + "<br/>\n</code>\n"
...
mainhead returns [String text]: tvoid MAIN LPARENTHESIS typedef1 RPARENTHESIS {$text = "<SPAN CLASS=\"palres\">" + $tvoid.type + "</SPAN> " + "<SPAN CLASS=\"ident\">" + $MAIN.text + "</SPAN>" +  "(" + $typedef1.text + ")";
                                                                                                                                      		};

code
	returns[String text]:
	{$text="";}
	| sent code {$text= $sent.text + $code.text;};
sent
	returns[String text]:
	asig SEMICOLON {$text="";}
	...
	| dowhile {$text="";};
asig
	returns[String text]: IDENTIFIER SASIGN exp; //
exp: factor exp1;
exp1: op factor exp1 |;
